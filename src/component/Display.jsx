...
import { deleteTransactions, editTransactions } from './Api/Api';
...
function Display(props) {
  const options = ["Food", "Rent", "Tip", "Medical", "Transportation"];
  ...
  const handleEditClick =async (itemKey) => {
    ...
    setCurrId(itemKey);
    const {data} = await axios.put(`${editTransactions}/${itemKey}`,{
  ...
  const handleEditSubmit = async (e) => {
    ...
    if(data.success === true){
      const updatedTransactions = props.data.map((item) => {
        if (item._id === currId) {
          return { ...item, ...values };
        }
        return item;
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                               { item.transactionType=="Credit"?<div class="inline-flex items-center px-3 py-1 rounded-full gap-x-2 text-emerald-900 bg-emerald-100/60 :bg-gray-800">
                                    ...
                                    <h2 class="text-sm font-normal">{item.transactionType}</h2>
                                </div>:
                                <div class="inline-flex items-center px-3 py-1 rounded-full gap-x-2 text-red-900 bg-red-100/60 :bg-gray-800">
                                    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M10 3L4.5 8.5L2 6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                            ...
                            <td class="px-4 py-4 text-sm text-gray-900 :text-gray-300 whitespace-nowrap">
                                <div class="flex items-center gap-x-2">
                                    ...
                                    <div>
                                        <h2 class="text-sm font-medium text-gray-800 :text-white ">{item.description}</h2>
                                    </div>
                                </div>
                            </td>
             ...
             <form className="w-full">
              ...
              <div className="mb-4">
               ...
               <div className="mb-4">
                 ...
                 <div className="md:w-32">
                   <select className="rounded-md font-karla h-10 max-sm:w-32" name="transactionType"    onChange={handleChange}>
                     <option>Please choose one option</option>
